...
function EdiDemand() {
    ...
    useEffect(() => {
        demandService.getDemand(id).then(res => {
            setDemand(res)
        })
    }, [])
    ...
    const editDemand = async (value) => {
        let isSuccess = await demandService.editDemand(value)
        if(isSuccess) {
            toast.success("Chỉnh sửa nhu cầu thành công")
            navigate("/")
        } else {
            toast.error("Chỉnh sửa nhu cầu thất bại.")
        }
    ...
    return (
        <>
            <Formik initialValues={demand} onSubmit={editDemand} validationSchema={Yup.object(objectValid)}>
                <Form className='container mt-5 shadow-sm p-3 rounded w-50 bg-white'>
                    ...
                    <div className="mt-3">
                        ...
                        <Field type="text" className="form-control" id="buyerCode" name="buyerCode" value={demand.buyer.name}
                               disabled/>
                    </div>
                    <div className="row mt-3">
                        <div className="col">
                            ...
                            <Field as="select" name="type" id="type" className="form-select">
                                <option>Chọn</option>
                                <option value="Mua" selected={demand.type==="Mua"}>Mua</option>
                                <option value="Thuê" selected={demand.type==="Thue"}>Thuê</option>
                            </Field>
                    ...
                    <div className="row mt-3">
                        <div className="col">
                            ...
                            <Field as="select" name="region" id="region" className="form-select" placeholder="Chọn">
                                <option>Chọn</option>
                                <option value="Trung tam" selected={demand.region==='Trung tam'}>Trung tâm</option>
                                <option value="Ngoai o" selected={demand.region==='Ngoai o'}>Ngoại ô</option>
                            </Field>
